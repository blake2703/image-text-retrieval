...
doubleA1 = readmatrix('A1.csv');
doubleA2 = readmatrix('A2.csv');

A1 = sparse(doubleA1);
A2 = sparse(doubleA2);


% Read node attributes matrices
doubleN1 = readmatrix('N1.csv');
doubleN2 = readmatrix('N2.csv');

N1 = sparse(doubleN1);
N2 = sparse(doubleN2);

% Read edge attributes matrices
doubleE1 = readmatrix('E1.csv');
doubleE2 = readmatrix('E1.csv');

sparseE1 = sparse(doubleN1);
sparseE2 = sparse(doubleN2);
...
fprintf('%d\n', ismatrix(H)); % Correct way to print the result of iscell



alpha = 0.90;  % Example decay factor
maxiter = 30; % Example maximum number of iterations
tol = 1e-4;    % Example tolerance
